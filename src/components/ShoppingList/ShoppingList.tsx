...
const ShoppingList: FC<ShoppingListProps> = (props) => {
  ...
  // This should be placed on the modal component itself
  ...
  // This functions should be inside a component called Sorter.jsx so they can reduce the ammount of code in ShoppingList.tsx
  function sortByName() {
    var itemListCloned = [...itemList];
  ...
  return (
    <>
      <div className={styles.ShoppingList}>
        <p>
          Sort by:
          <button className="btnlink" onClick={sortByName}>
            Name
          </button>
          <button className="btnlink" onClick={sortByPrice}>
            Price
          </button>
        </p>
                ...
                <li className="form-check" key={index}>
                  <div>
                    <input
                      ...
                      onChange={(e) => {
                        isCompleted(index, e.target.checked);
                      }}
                    />
                    <label className="form-check-label">{item.name}</label>
                    <label className="form-check-label bold mx-2">
                      {item.price} SEK
                    </label>
                  </div>
        ...
        <div className="w-100 text-center mb-3">
          <button
            className="primaryBtn d-block m-auto"
      ...
      <div className={styles.CompletedList}>
        <button
          className="btnlink mb-3"
          onClick={() => setShowList(!showList)}
          disabled={itemList.some((a) => a.isCompleted == true) ? false : true}
        >
